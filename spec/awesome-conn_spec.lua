...
--                           t["servicepath"])
          ...
          assert.are.equal("150"              , t["Ethernet"]["MTU"      ])

          assert.are.equal(4              ,#t["IPv4"           ])
          ...
          assert.are.equal(1     ,#t["IPv4.Configuration"          ])
          assert.are.equal("dhcp", t["IPv4.Configuration"]["Method"])

          assert.are.equal(0, #t["IPv6"])
          
          assert.are.equal(2         ,#t["IPv6"                         ])
          assert.are.equal("auto"    , t["IPv6.Configuration"]["Method" ])
          assert.are.equal("disabled", t["IPv6.Configuration"]["Privacy"])
                           
          assert.are.equal(2, #t["Nameservers"])
          assert.are.equal("192.168.32.102", t["Nameservers"][1])
          assert.are.equal("192.168.24.132", t["Nameservers"][2])

          assert.are.equal(0, #t["Nameservers.Configuration"])
          
          assert.are.equal(1, #t["Timeservers"])
          assert.are.equal("10.0.2.2", t["Timeservers"][1])
          
          assert.are.equal(0, #t["Timeservers.Configuration"])

          assert.are.equal(1, #t["Domains"])
          assert.are.equal("domain.com", t["Domains"][1])

          assert.are.equal(0, #t["Domains.Configuration"])

          assert.are.equal(1, #t["Proxy"])
          assert.are.equal("direct", t["Proxy"]["Method"])

          assert.are.equal(0, #t["Proxy.Configuration"])
          assert.are.equal(0, #t["Provider"])
    end)
  end)
  -- }}}
end)
